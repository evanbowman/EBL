;;; ebl.el
;;; This is a hack of scheme-mode, to support syntax highlighting in ebl, which
;;; is similar to scheme.
;;
...
(defvar ebl-mode-abbrev-table nil)
(define-abbrev-table 'ebl-mode-abbrev-table ())
...
(defun ebl-mode-variables ()
  (set-syntax-table ebl-mode-syntax-table)
  (setq local-abbrev-table ebl-mode-abbrev-table)
  ...
  (setq mode-line-process '("" ebl-mode-line-process))
...
;; Used by cmuscheme
(defun ebl-mode-commands (map)
  ;;(define-key map "\t" 'indent-for-tab-command) ; default
  (define-key map "\177" 'backward-delete-char-untabify)
  (define-key map "\e\C-q" 'indent-sexp))

;;;###autoload
(define-derived-mode ebl-mode prog-mode "Scheme"
...
(defcustom ebl-mode-hook nil
  "Normal hook run when entering `ebl-mode'.
See `run-hooks'."
  :type 'hook
  :group 'scheme)
       ...
       (concat
        "(" (regexp-opt
             '("begin" "call-with-current-continuation" "call/cc"
               ...
               "do" "else" "for-each" "if" "lambda" "λ" "namespace" "recur" "mutable"
...
(defconst scheme-sexp-comment-syntax-table
  (let ((st (make-syntax-table ebl-mode-syntax-table)))
    (modify-syntax-entry ?\; "." st)
    (modify-syntax-entry ?\n " " st)
    (modify-syntax-entry ?#  "'" st)
    st))
...
;;;###autoload
(define-derived-mode dsssl-mode ebl-mode "DSSSL"
...
Blank lines separate paragraphs.  Semicolons start comments.
\\{ebl-mode-map}
Entering this mode runs the hooks `ebl-mode-hook' and then
...
;; shouldn't cause much trouble in ebl-mode.
