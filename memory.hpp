...
public:
    ...
    public:
        ...
        template <typename T>
        Ptr<T> cast() const { return {handle_}; }
    ...
    public:
        template <typename U>
        Ptr(Ptr<U> other) : GenericPtr{other.handle()} {
...
public:
    template <typename U>
    Local(Environment& env, Heap::Ptr<U> value) : value_(value) {}

    template <typename U>
    Local& operator=(Heap::Ptr<U> value) {
        value_ = value;
        return *this;
    }
...
template <typename T>
Heap::Ptr<T>::Ptr(Local<T> other) : Ptr(other.get()) {}

template <typename T>
Heap::Ptr<T>::Ptr(Persistent<T> other) : Ptr(other.value_) {}
