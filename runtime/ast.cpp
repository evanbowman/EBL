...
{
    cachedVal_ = storeI<ebl::Character>(*env.getContext(), value_);
}


void String::init(Environment& env, Scope& scope)
{
    cachedVal_ = storeI<ebl::String>(*env.getContext(), value_);
}

void Symbol::init(Environment& env, Scope& scope)
{
    auto strLoc = storeI<ebl::String>(*env.getContext(), value_);
    auto str = env.getContext()->immediates()[strLoc];
    cachedVal_ = storeI<ebl::Symbol>(*env.getContext(), str.cast<ebl::String>());
}
...
void Pair::init(Environment& env, Scope& scope)
{
    ...
    auto p = env.create<ebl::Pair>(ctx->immediates()[first_->cachedVal_],
...
{
    cachedVal_ = storeI<ebl::Integer>(*env.getContext(), value_);
}


void Float::init(Environment& env, Scope& scope)
{
    cachedVal_ = storeI<ebl::Float>(*env.getContext(), value_);
}
    ...
    dynamicWind(
        [&] {
            if (not docstring_.empty()) {
                cachedDocstringLoc_ =
                    storeI<ebl::String>(*env.getContext(), docstring_);
            }
